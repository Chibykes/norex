        ...
        <div className={`p-4 fixed inset-0 w-full h-full bg-black z-[1] overflow-auto ${openModal ? 'block':'hidden'}`}>
            ...
            <div className="py-3">
                {countriesList.sort(sorter).map(({code, name, price, details, flag, country}: Country):ReactNode => (
                    <div key={code} className="flex justify-start items-center gap-4 border-b border-[#111] py-2 cursor-pointer" onClick={() => handleListSelect({code, details, name, price, flag, country})}>
            ...
            <div className="fixed bottom-4 left-0 right-0 flex justify-center items-center gap-2 w-1/5 mx-auto p-2 bg-[#222] font-semibold text-sm rounded-sm shadow-lg" onClick={() => setOpenModal(false)}>
